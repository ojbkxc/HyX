...
多链路极速 P2P 文件传输。基于 QUIC，Rust 实现高速传输内核，Android 原生界面。

## 特性

- **QUIC 内核**：多链路聚合、零往返建连、断点续传
- **单一路径引擎**：A/B 两个传输引擎融合为一条连续流，去逐块 fsync 与逐块确认
- **性能优化**：BBR 拥塞控制、滑动窗口、GSO 报文合并、收端后台写盘重叠网络读、压缩脱离 async 执行器
- **数据完整性**：增量 SHA-256 校验
- **局域网自动发现**：UDP beacon 扫描附近设备
- **配对互传**：扫码 / 配对码走 rendezvous 建立连接
